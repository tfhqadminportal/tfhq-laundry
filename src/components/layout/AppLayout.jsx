...
import {
  ...
  FileBarChart2, PlusCircle, History, Menu,
} from 'lucide-react'
...
export default function AppLayout() {
  ...
  const mobileNavItems = isAdmin ? adminMobileNav : staffMobileNav
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      {/* Mobile Sidebar Overlay (for edge-cases / extra nav on admin) */}
      {open && (
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Mobile top bar — slim brand header only */}
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-navy-800 border-b border-navy-700">
          ...
          {/* "More" button — shows full sidebar if ever needed */}
          <button
        ...
        {/* Page content — padded bottom so it's never hidden behind the nav bar */}
